...
- **E2E admin login must use `Promise.all([waitForNavigation, click])`** — a bare `page.click('#wp-submit')` races with the login redirect. `waitForSelector('#wpadminbar')` after a bare click can time out on slow CI runners. Fix: wrap the click and navigation wait in `Promise.all`, then wait for `#wpadminbar` with a 20s timeout.
