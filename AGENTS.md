...
- `includes/` — PSR-4 classes (`Stampy\` namespace).
  ...
  - `Admin/` — admin menu, subscribers list table + detail view, lists list table + CRUD, SMTP settings page + test-send, campaign preview handler. List creation redirects to list overview (not edit view) after save.
  ...
  - `Campaigns/` — Campaign CPT registration (postmeta, block restriction, editor data localization) + email renderer (blocks → table-based HTML + plain text + auto-append unsubscribe footer).
...
- `src/` — TypeScript/JS (block editor, frontend).
  ...
  - `campaign-editor/` — Campaign editor sidebar plugin (block.json, index.tsx). Uses `@wordpress/plugins`, `@wordpress/editor`, `@wordpress/data` (WordPress externals, types in `types/wordpress-plugins.d.ts`).
