...
- `includes/` — PSR-4 classes (`Stampy\` namespace).
  ...
  - `Admin/` — admin menu, subscribers list table + detail view, lists list table + CRUD, SMTP settings page + test-send, campaign preview handler, campaign send/cancel/progress AJAX handlers + row actions. List creation redirects to list overview (not edit view) after save. Campaign send UI is fully React-based (no PHP meta box).
  ...
  - `Campaigns/` — Campaign CPT registration (postmeta, block restriction, editor data localization) email renderer (blocks → table-based HTML + plain text + auto-append unsubscribe footer), merge-tag registry (per-recipient personalization), sending engine (audience resolution, snapshot, AS batches, claiming, failure marking, RFC 8058 headers, progress UI).
