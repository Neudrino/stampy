...
```
stampy/
├── .github/            # workflows/ (stampy-ci.yml, release.yml, codeql.yml),
...
├── SECURITY.md  .distignore  LICENSE (GPLv3)  README.md  readme.txt
```
...
**`.github/workflows/stampy-ci.yml` — every PR + push to `main`**
...
**`.github/workflows/release.yml` — on semantic version tag push (e.g. `1.0.0`):**
...
slug permanent, GPLv3+ throughout); afterwards releases are "merge, tag, done".
...
version it carries. The `release.yml` workflow (triggered by a semantic version tag,
...
| 17 | WP.org submission & release pipeline: submit the `stampy` slug to WordPress.org for review and enable automated SVN deployment. Ships a self-contained production zip (no Composer/npm needed on the user's server) built via `.distignore`-filtered staging + `npm run build` + `composer install --no-dev`. The official `wordpress/plugin-check` tool (the same one the WP.org review team uses) gates the release at three levels — CI, the release workflow, and an E2E test — all must report 0 errors. Two GitHub workflows: `stampy-release.yml` (tag-triggered: version consistency check → build → Plugin Check on staged artifact → GitHub Release with zip) and `stampy-wp-deployment.yml` (manual `workflow_dispatch` in a protected `wporg-deploy` environment → SVN deploy). Plugin assets (banner/icon/screenshots) live in `.wordpress-org/` and deploy to the SVN `assets/` directory. **Purpose:** without this phase, the plugin only exists as a GitHub repo — it cannot be installed via the WP.org plugin directory, cannot receive automatic updates, and is invisible to the WP.org search and update infrastructure | Plugin Check 0 errors (CI + E2E + release pipeline); plugin live; tag-deploy works | install from WP.org on a clean site |
| 18 | Pipeline renaming: prefix all GitHub workflow filenames with `stampy-` (`ci.yml` → `stampy-ci.yml`, `build-release.yml` → `stampy-release.yml`, `deploy-wporg.yml` → `stampy-wp-deployment.yml`) for consistency and discoverability in the Actions UI. Documentation (AGENTS.md, README.md, PROGRESS.md) updated to reference the new names. No functional or code changes | `npm run validate:fast` green; CI workflows trigger correctly under new names | push a PR → all three workflows appear in the Actions tab with `stampy-` prefix |
| 19 | Pre-publish review & cleanup: resolve findings from a thorough pre-publish audit. Remove committed dev TLS certs from git tracking (gitignored only). Fix i18n gaps (untranslated AJAX error strings in `CampaignSendPage`). Add WP nonce to the preferences POST form (defense-in-depth alongside the existing HMAC-signed URL). Convert `Privacy.php` `IN()` clause to prepared statement. Remove dead code (`STAMPY_VERSION` export, `Migrations.php` — no published versions exist to migrate from, so the migration runner is YAGNI; `Schema::DB_VERSION` reset to 1, `Installer` calls `Schema::install()` directly). Tighten `.distignore` to exclude dev-only files (`.husky/`, config files, `package-lock.json`, etc.). Fix stale version references in `README.md` | `npm run validate:fast` green; `npm run test:integration:php` green; `npm run test:e2e` green | `git ls-files dev/certs/` returns empty; production zip contains no dev files |
...
registry, generic attribute storage, block deprecation path), so none of
them requires a contract or schema break.

The field management, spam guards (quiz + Turnstile + Friendly Captcha),
and submission log were originally post-1.0 roadmap items but have been
promoted into the v1 MVP phases (Phases 11–16). The WP.org submission &
release pipeline is Phase 17, pipeline renaming is Phase 18, and the
pre-publish review & cleanup is Phase 19. The remaining post-1.0 items are:

- **Central form builder** (originally Phase 18, deferred to post-v1):
  `stampy_form` CPT edited in the block editor with dedicated field blocks
  (text, textarea, number, date, select, radio, checkbox-group, hidden,
  consent). A "Stampy Form" picker block embeds any form on any page.
  Per-form settings: target lists, success message, redirect URL, per-field
  error messages. Server-side validation parses the form's blocks and
  validates each submission against that exact schema. The v1 signup block
  migrates via a block `deprecated[]` entry. Architecture-ready: the REST
  signup contract accepts `form_id` from day one, the `ValidatorRegistry`
  is extensible, and the GDPR exporter walks `subscriber_meta` generically.
