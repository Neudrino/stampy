...
{prefix}_fields               id, field_key (UNIQUE), label, type
                              ...
                              — v1 note: `required` is NOT enforced by the
                                ...
                                name inputs optional); the Phase 18 form
...
| 4 | Signup block in TSX (email + optional first/last-name inputs, consent, list selection **requiring ≥1 list** — editor notice + front-end no-op if none, a11y; attributes deprecation-ready for the Phase 18 form builder) | Jest; first E2E signup→confirm | place block on a page at `:8888`, select a list, sign up in the browser, confirm via `:8025` |
...
| 17 | WP.org submission & release pipeline: `stampy` slug submission (search shows no existing `stampy` plugin — slug likely free, confirmed only at submission; have a fallback slug ready), review, enable auto-deploy. **Packaging & delivery:** the plugin is delivered via the WordPress.org SVN repository, not as a raw zip. SVN layout: `trunk/` (development code), `tags/<version>/` (one subdirectory per release — this is what users download), `assets/` (screenshots, banner, icon — served via CDN, NOT shipped in the plugin zip). The directory reads `trunk/readme.txt` `Stable tag:` to resolve the download version, then serves files from `tags/<stable-tag>/`. The displayed version number comes from the main PHP file `Version:` header. Both the trunk and the tag's readme.txt must have the correct Stable Tag or updates break. **Zip contents:** the plugin zip = a complete, self-contained plugin that runs without Composer/npm on the user's server. Must include `stampy.php` (at the root, NOT in a subfolder), `readme.txt` (WP.org readme format), `includes/`, `vendor/` (Composer prod deps committed), `build/` (compiled JS/CSS), `assets/captcha-loader.js` (runtime assets), `languages/` (POT/PO/MO), `LICENSE`. Must NOT include tests, config files, dev tooling, `node_modules/`, `.github/`, `.wordpress-org/` (screenshots go to SVN `assets/` instead). **readme.txt validation:** the readme must pass the WP.org readme validator (`https://wordpress.org/plugins/developers/readme-validator/`). **Plugin Check (official `wordpress/plugin-check` tool, https://github.com/WordPress/plugin-check):** this is the tool the WP.org review team uses to vet submissions — all checks it reports must pass (0 errors). It runs both **static checks** (code analysis: late escaping, direct file access, i18n usage, code obfuscation, plugin header fields, readme validation, file types, enqueued resources, etc.) and **runtime checks** (actually executes the plugin in an isolated WP environment: enqueued-scripts-size, enqueued-styles-size, non-blocking-scripts, etc.). Plugin Check is integrated at three levels: (1) **CI `ci.yml`** — the existing `wordpress/plugin-check-action@v1` job runs on every PR and push against the repo root with dev files excluded; this catches issues early during development. (2) **Release pipeline `build-release.yml`** — after staging the production artifact (`.distignore`-filtered, `npm run build` + `composer install --no-dev`), Plugin Check runs against the **staged artifact directory** (not the repo root), so no manual file/directory exclusions are needed — the staged dir contains only production files. This is the authoritative gate: if Plugin Check fails here, the release is blocked. Both static and runtime checks run (the action's `wp-env` instance provides the full WP environment for runtime checks, and the `--require=cli.php` flag enables the runtime-check bootstrap). The `plugin_repo` category (all submission-guideline checks) is explicitly included. (3) **E2E test suite** — an E2E test (`tests/e2e/plugin-check.spec.ts`) runs `wp plugin check stampy --format=json --require=./wp-content/plugins/plugin-check/cli.php` inside the wp-env tests container (where the Plugin Check plugin is pre-installed), asserts exit code 0 and no errors in the JSON output. This ensures Plugin Check passes in the local dev environment before pushing a release tag. **Release pipeline (two GitHub workflows):** (A) `build-release.yml` — triggered by a `v*` tag push: version consistency check (tag == `stampy.php` Version == `readme.txt` Stable tag), `npm ci && npm run build`, `composer install --no-dev --optimize-autoloader`, stage to clean dir via `.distignore`, run Plugin Check (`wordpress/plugin-check-action@v1` with `build-dir` pointing at the staged artifact, `categories=plugin_repo`, no file exclusions needed since the staged dir is production-only), create zip, create GitHub Release with zip attached. (B) `deploy-wporg.yml` — triggered manually (`workflow_dispatch`) after the GitHub Release is reviewed: runs in a protected `wporg-deploy` environment with required reviewers, deploys via `10up/action-wordpress-plugin-deploy@stable` with `SLUG=stampy` and `ASSETS_DIR=.wordpress-org` (screenshots/banner/icon), SVN credentials from environment secrets. The manual-dispatch gate prevents accidental SVN pushes. **Plugin assets (`.wordpress-org/` dir):** banner (`banner-772x250.png`, `banner-1544x500.png`), icon (`icon-128x128.png`, `icon-256x256.png`, optionally `icon.svg`), screenshots (`screenshot-1.png` etc. matching the `readme.txt` screenshot list). These are excluded from the plugin zip (`.distignore` lists `.wordpress-org`) and deployed to the SVN `assets/` directory. **First-time submission flow (before the deploy workflow can run):** bump version from `0.0.1` to `1.0.0`, push `v1.0.0` tag → download the zip artifact → submit at `https://wordpress.org/plugins/developers/add/` with a brief description → wait for manual review (1–10 business days) → after approval, set up `SVN_USERNAME`/`SVN_PASSWORD` secrets in the GitHub `wporg-deploy` environment → run the deploy workflow to push to SVN for the first time. **Guidelines compliance:** GPL-2.0-or-later, no code obfuscation, no external executable code loading, no tracking without consent, no bundling of WP-bundled libraries, human-readable code, version numbers incremented per release. **Purpose:** without this phase, the plugin only exists as a GitHub repo — it cannot be installed by regular WordPress users via the plugin directory, cannot receive automatic updates, and is invisible to the WP.org search and update infrastructure | Plugin Check 0 errors (CI + E2E + release pipeline); plugin live; tag-deploy works | install from WP.org on a clean site |
| 18 | Central form builder: **Stampy → Forms** admin screen with a new `stampy_form` CPT edited in the block editor. Dedicated field blocks: text, textarea, number, date, select, radio, checkbox-group, hidden, consent — each with its own `block.json` + `edit.tsx` + `save.ts`. A **"Stampy Form" picker block** embeds any form on any page — one form reusable everywhere, edited once, updated everywhere. Per-form settings: target lists (fixed or visitor-selectable), success message, optional redirect URL, per-field custom error messages. Server-side validation resolves `form_id` → parses the form post's blocks → validates the submission against that exact schema. The v1 signup block migrates via a block `deprecated[]` entry. **Purpose:** v1 ships a single, fixed signup block — email + optional first/last name + consent checkbox + list selector. Every signup form across the entire site looks the same and collects the same fields. If an operator wants a "Newsletter" form that only asks for email, a "Webinar registration" form that asks for email + company + job title, or a "Free trial" form that asks for email + name + phone + accepts different terms — they cannot do it. There is one block, one hardcoded field schema, one set of fields. The form builder solves this: operators compose forms visually in the block editor using field blocks, each form has its own schema (which fields, which types, which are required, which lists it targets), and the picker block embeds any form on any page. The server-side validator parses the form's blocks to validate each submission against that exact schema. This is the native replacement for what Contact Form 7 would otherwise provide — multiple reusable, customizable forms with per-form field composition, target lists, and validation rules — with no CF7 dependency. The REST signup contract (`form_id` + `fields`) already accepts `form_id` from day one, the `ValidatorRegistry` already supports extension (Phase 13 added textarea/number/date/select/checkbox validators), and the v1 signup block's attributes are deprecation-ready — so no contract or schema break is needed | integration tests (form CPT registration, field block rendering, server-side block-parsing validation per field type, picker block embedding, signup via custom form creates subscriber with correct fields + list memberships, per-form settings honored); Jest (field block edit components); E2E | create a "Webinar Registration" form with email + company + job title fields → embed it on a page via the picker block → fill it out in the browser → verify the subscriber is created with all custom attributes and added to the correct list |
...
but have been promoted into the v1 MVP phases (Phases 11–16). The WP.org
submission & release pipeline is Phase 17. The form builder remains Phase 18
(post-v1, architecture-ready). The remaining post-1.0 items are:
